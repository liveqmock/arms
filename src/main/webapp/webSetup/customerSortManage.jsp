...
<body>
	<table border="0" style="width: 100%;">
		<tr>
			<td class="titlebg"><span>基础设置</span> <span class="titleSpan">(客户类型管理)</span>
			</td>
		</tr>
	</table>
	<table id="mydg" class="easyui-datagrid"
		data-options="url:'queryCustomerSort.action',
						   rownumbers:true,
						   singleSelect:true,
						   toolbar:'#toolbar',
						   pagination:false">
		<thead>
			<tr>
				<th width="200" field="txtName">客户类型名称</th>
	...
	<div id="toolbar">
		<a href="javascript:void(0)" class="easyui-linkbutton"
			iconCls="icon-add" plain="true" onclick="addItem()">新增客户类型</a>
	</div>
	<div id="mydlg" class="easyui-dialog" closed="true"
		...
		<form name="fm" method="post" id="fm">
			<table border="0">
				<tr>
					<td align="right"><span class="requireSpan">*&nbsp;</span>客户类型名称:</td>
					<td><input name="txtName" type="text" maxlength="100"
						class="easyui-textbox"
						data-options="required:true"
						id="txtName" style="width: 250px;" /></td>
				</tr>
				<tr>
	...
	<script type="text/javascript">
		var url;
		function addItem() {
			$('#mydlg').dialog('open').dialog('setTitle', '添加客户类型');
			$('#fm').form('clear');
			url = 'insertCustomerSort.action';
		}
		function editItem(clickevent) {
			var row = $('#mydg').datagrid('getEventTargetRow', clickevent);
			if (row) {
				$('#mydlg').dialog('open').dialog('setTitle', '修改客户类型');
				$('#fm').form('load', row);
				url = 'updateCustomerSort.action?txtGuid=' + row.txtGuid;
			}
		}
		function deleteItem(clickevent) {
			...
			if (row) {
				$.messager.confirm('确认', '确定要删除选中信息吗?', function(r) {
					if (r) {
						$.post('deleteCustomerSort.action', {
							"txtGuid" : row.txtGuid
						}, function(result) {
							if (result.errorMsg) {
