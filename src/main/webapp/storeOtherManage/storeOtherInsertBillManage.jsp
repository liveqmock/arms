...
<body>
	<table border="0" style="width: 100%;">
		<tr>
			<td class="titlebg"><span>物料管理</span><span class="titleSpan">(采购管理)</span></td>
	...
	<form name="fmSearch" method="post" id="fmSearch" class="searchform">
		<table id="searchPanel" class="searchPanel">
			<tr>
				<td nowrap>出入库时间:</td>
				<td><input name="txtRuKuDateBegin" type="text"
					id="txtRuKuDateBegin" class="easyui-datebox" style="width: 100px;" /></td>
				<td>至</td>
				<td><input name="txtRuKuDateEnd" type="text"
					id="txtRuKuDateEnd" class="easyui-datebox" style="width: 100px;" /></td>
				<td nowrap>单号:</td>
				<td><input name="txtBillNo" type="text" class="easyui-textbox"
					maxlength="20" id="txtBillNo" style="width: 100px;" /></td>
				<td nowrap>供应商:</td>
	...
	<table id="mydg" class="easyui-datagrid"
		data-options="url:'queryRuKuDanWuLiao.action?saleAfterWeiXiuGuid=<s:property value='saleAfterWeiXiuGuid' />&action=<s:property value='action' />',
						   rownumbers:true,
                           toolbar:'#tb',
	...
	<script type="text/javascript">
		...
		function doSearch() {
			$("#fmSearch")
					.form(
							'submit',
							{
								url : "queryRuKuDanWuLiao.action?saleAfterWeiXiuGuid=<s:property value='saleAfterWeiXiuGuid' />&action=<s:property value='action' />",
		...
		function addCheckedCaiGouWuLiao() {
			...
			if (checkedRows.length == 0) {
				...
				var url = "<s:property value='basePath' />/saleAfterManage/addGongDanWeiXiuWuLiaoFromCaiGou.action?d="
						+ new Date();
				$
						.post(
								url,
